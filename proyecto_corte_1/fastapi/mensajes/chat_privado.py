...
# Crear cita
@app.post("/mensajes")
async def crear_mensaje(numero_cel_origen : str, numero_cel_destino : str, mensaje : str, ):    
    
    id_usuario_origen = validarExistencia(numero_cel_origen, "usuario origen")
    id_usuario_destino = validarExistencia(numero_cel_destino, "usuario destino")

    print("TIPO DE DATO:", type(id_usuario_origen))
    print("VALOR:", id_usuario_origen)
    ...
    try:
        ...
        await cursor.execute(query, (id_usuario_origen, id_usuario_destino, mensaje, 'ver', 'privado',))
        await conn.commit()

        await cursor.close()
        conn.close()

        return {"mensaje": "Mensaje creado correctamente"}
    except Exception as e:
        # Manejo de errores básico
        raise HTTPException(status_code=500, detail=f"Error al registrar el mensaje: {str(e)}")
